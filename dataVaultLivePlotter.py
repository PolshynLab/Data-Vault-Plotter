    ...
    def openLivePlots(self, twoPlots, onePlots, fresh, c = None):
        try: 
            ...
            for plot in twoPlots:
                new2DPlot = plot2DWindow(self.reactor, twoPlots[plot], self.listenTo, self.listenPlotFile, x0, y0, fresh, self)
                new2DPlot.show()
                y0 += 50
            x0, y0 = 1250, 25
            for plot in onePlots:
                new1DPlot = plot1DWindow(self.reactor, onePlots[plot], self.listenTo, self.listenPlotFile, x0, y0, fresh, self)
                new1DPlot.show()
                y0 += 50
